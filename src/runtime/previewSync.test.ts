...
function previewStatus(state: RuntimePreviewState, stale = false): RuntimePreviewStatus {
  return {
    ...
    controlRevision: state === "stopped" ? null : 0,
    previewControlRevision: state === "stopped" ? null : 0,
    controlLive: state !== "stopped",
    lastControlUpdateAt: state === "stopped" ? null : "unix-ms:1",
    stale,
